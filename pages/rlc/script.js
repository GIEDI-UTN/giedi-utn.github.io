class RLCSimulator {
    ...
    calculateValues() {
        ...
        return {
            R, L, C, omega, V0, XL, XC, Z, phi, f, T,
            Ipeak, Irms, VR, VL, VC, S, P, Q
        };
    }
    ...
    update() {
        ...
        this.valueElements.phase.textContent = (values.phi * 180 / Math.PI).toFixed(1) + '°';
    ...
    updateDominanceIndicator(values) {
        ...
        if (Math.abs(XL - XC) < 0.01) {
            text = 'Estado del circuito: En resonancia. XL = XC. Voltaje y corriente en fase (φ = 0°)';
        } else if (XL > XC) {
            text = `Estado del circuito: Inductivo. XL > XC. El voltaje adelanta a la corriente por ${phaseDegrees}°`;
        } else {
            text = `Estado del circuito: Capacitivo. XC > XL. El voltaje atrasa a la corriente por ${Math.abs(phaseDegrees)}°`;
        }
    ...
    setResonance() {
        ...
        if (omega0 > 200) {
            ...
            this.resonanceMessage.innerHTML = `Frecuencia de resonancia: ${omega0.toFixed(1)} rad/s`;
    ...
    drawImpedanceTriangle(values) {
        ...
        ctx.fillText(`φ = ${(phi * 180 / Math.PI).toFixed(1)}°`, x2 - 30, y2 - 10);
    }
    ...
    drawWaveforms() {
        ...
        const drawWave = (amplitude, phase, color, label) => {
            ...
            ctx.fillStyle = color;
            ctx.font = '12px Arial';
            ctx.fillText(label, width - 100, centerY - amplitude * Math.sin(omega * timeRange + phase) - 10);
        };
        
        // Dibujar las ondas
        drawWave(amplitude * (Ipeak / Math.max(Ipeak, V0/50)), 0, '#3498db', 'i(t)');
